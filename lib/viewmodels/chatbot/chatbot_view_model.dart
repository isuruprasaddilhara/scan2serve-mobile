...
class ChatbotViewModel extends ChangeNotifier {
  ChatbotViewModel() {
    ...
    if (ChatbotMenuContextStore.instance.shouldAnnounceMenuRefresh()) {
      _entries.add(
        ChatTextBubble(
          isUser: false,
          text: ChatbotMenuContextStore.instance.buildAssistantMenuSyncLine(),
        ),
      );
      ChatbotMenuContextStore.instance.markMenuAnnouncedToUser();
    }
  ...
  Future<void> submitFaqTopicAnswer(String displayLabel, String apiPrompt) async {
    ...
    notifyListeners();
    try {
      final String reply = await _postChatWithMenuContext(apiPrompt);
  ...
  Future<void> _submitUserMessage(String text) async {
    ...
    try {
      final reply = await _postChatWithMenuContext(t);
      _entries.add(ChatTextBubble(isUser: false, text: reply));
  ...
  Future<String> _postChatWithMenuContext(String message) async {
    final String prefix = ChatbotMenuContextStore.instance.buildPromptPrefix();
    if (prefix.isEmpty) {
      return postChatMessage(message);
    }
    return postChatMessage('$prefix$message');
  }
