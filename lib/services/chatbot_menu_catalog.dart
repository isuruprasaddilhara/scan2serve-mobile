...
bool _matchesVeg(MenuItemModel m) {
  ...
  if (_hasMeatProtein(s)) return false;
  if (RegExp(r'\b(veg|veggie)\b').hasMatch(s)) return true;
  if (s.contains('vegetarian') || s.contains('vegan')) return true;
  if (s.contains('vegetable')) return true;
  if (s.contains('tofu') || s.contains('paneer')) return true;
  return false;
}
